...
(color-coded green/yellow/red), exact token counts, the session ID, plus the
familiar `user@host:dir (branch)` prefix.

Example output:

```
jos@laptop:myproj (master) Opus 4.7 (1M context) | context used 12.3% - (123,456/1,000,000)
session: 0f3c…
```
